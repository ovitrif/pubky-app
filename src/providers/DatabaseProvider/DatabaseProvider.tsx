  ...
  if (error) {
    ...
    content = (
      <Container overrideDefaults className="flex min-h-screen flex-col items-center justify-center gap-4">
        <Spinner />
        <p className="text-sm text-foreground">Loading local storage…</p>
        <a href="?storageRetry=1" className="text-sm text-brand underline">
          Reload if this takes more than a few seconds
        </a>
      </Container>
    );
  } else {
