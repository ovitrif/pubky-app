...
| WEB-02 | XSS steals local bearer/session data                 | Account/entitlement compromise | Marketplace JSON CSP `default-src 'none'`; Next.js enforced nonce CSP via `src/proxy.ts`; raw runtime-config script is nonce'd (ADR 0017); no unsafe HTML |
